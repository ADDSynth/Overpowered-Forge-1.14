...
public final class NoteButton extends AbstractButton {
  ...
  public NoteButton(int x_position, int y_position, int track, int frame, String note){
    super(x_position, y_position , GuiMusicBox.note_button_width, GuiMusicBox.note_button_height, note);
  ...
  public final void drawButton(final Minecraft mc, final int mouseX, final int mouseY, final float partial){
    if(visible){
      if(getMessage() != null){
        mc.fontRenderer.drawString(getMessage(), x + center_x - (mc.fontRenderer.getStringWidth(getMessage()) / 2), y + text_draw_y, text_color);
      }
    }
  }
  ...
  public final boolean mousePressed(final Minecraft mc, final int mouseX, final int mouseY){
    return active && mouseX >= x && mouseX < x + width && mouseY >= y && mouseY < y + height;
  }
