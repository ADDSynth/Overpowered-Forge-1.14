...
public final class JavaUtils {
  ...
  public static final <T> T getArrayValue(final T[] array, final int index, final boolean warn){
    if(array == null){
      throw new NullPointerException("Input array for "+JavaUtils.class.getName()+".getArrayValue() is null!");
    }
    final int length = array.length;
    if(length == 0){
      throw new IllegalArgumentException("Input array for "+JavaUtils.class.getName()+".getArrayValue() doesn't have any values!");
    }
  ...
  public static final <T> T getArrayValue(final T[] array, final int index, final T default_value, final boolean warn){
    if(array == null){
      if(warn){
        ADDSynthCore.log.error(new NullPointerException("Input array for "+JavaUtils.class.getName()+".getArrayValue() is null!"));
      }
      return default_value;
    }
    final int length = array.length;
    if(length == 0){
      if(warn){
        ADDSynthCore.log.error(new IllegalArgumentException("Input array for "+JavaUtils.class.getName()+".getArrayValue() doesn't have any values!"));
      }
      return default_value;
    }
    if(isInsideBounds(index, length) == false){
  ...
  // FEATURE: add a check_array() function, that returns true if index is valid.
